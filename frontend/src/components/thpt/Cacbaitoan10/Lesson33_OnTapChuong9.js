...
export default function Lesson33_OnTapChuong9() {
  ...
  const renderTheory = ({ t, lang, LessonVideoPlayer, videoId, videoSubtitles }) => {
    ...
    <div className="reveal" data-reveal data-reveal-stagger data-stagger="70" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:40,transition:"all 0.3s"}}>
      {[{slug:"toa-do-vecto",num:"31",title:t("Tọa Độ Vectơ","Vector Coords")},{slug:"duong-thang",num:"32",title:t("Đường Thẳng","Lines")},{slug:"duong-tron",num:"33",title:t("Đường Tròn","Circles")},{slug:"elip",num:"34",title:"Elip"}].map(l=>(<Link key={l.slug} href={`/cacbailam10/${l.slug}`} style={{textDecoration:"none"}}><article style={{padding:12,borderRadius:10,background: "rgba(255, 255, 255, 0.04)", border: "1px solid rgba(255, 255, 255, 0.08)",boxShadow:"0 4px 12px rgba(0,0,0,0.1)",cursor:"pointer"}}><div style={{fontSize:11,color: "rgba(255, 255, 255, 0.5)",marginBottom:2}}>{t("Bài","L")} {l.num}</div><div style={{fontSize:13,fontWeight:600,color: "#22d3ee"}}>{l.title}</div></article></Link>))}
    </div>
    <section id="tomTat" style={{scrollMarginTop:80,marginBottom:64}}><SH icon="📚" title={t("Tóm Tắt Chương IX","Chapter IX Summary")} />
      <div className="reveal" data-reveal data-reveal-stagger data-stagger="80" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:18,transition:"all 0.3s"}}>
        ...
        ].map((card,i)=>(<article key={i} style={{padding:18,borderRadius:10,background: "rgba(255, 255, 255, 0.04)", border: "1px solid rgba(255, 255, 255, 0.08)",boxShadow:"0 4px 12px rgba(0,0,0,0.1)"}}><div style={{fontSize:14,fontWeight:700,color: "#22d3ee",marginBottom:10}}>{card.title}</div>{card.pts.map((pt,j)=><div key={j} style={{fontSize:12,color: "rgba(255, 255, 255, 0.7)",marginBottom:6,display:"flex",gap:6}}><span style={{color: "#22d3ee",fontWeight:700,flexShrink:0}}>•</span><span style={{fontFamily:"monospace"}}>{pt}</span></div>)}</article>))}
      </div>
    </section>
