  ...
  const renderTheory = ({ t, lang, LessonVideoPlayer, videoId, videoSubtitles }) => {
    ...
    <section id="k1" style={{scrollMarginTop:80,marginBottom:64}}><SH icon="📖" title={t("1. Phương Trình Tổng Quát","1. General Equation")} />
      <div className="reveal" data-reveal style={{padding:20,borderRadius:10,background: "rgba(255, 255, 255, 0.04)", border: "1px solid rgba(255, 255, 255, 0.08)",boxShadow:"0 4px 12px rgba(0,0,0,0.1)",marginBottom:20}}>
        <div style={{background: "rgba(255, 255, 255, 0.04)", border: "1px solid rgba(255, 255, 255, 0.08)",borderRadius:10,padding:"14px 18px",fontFamily:"monospace",fontSize:17,textAlign:"center",color: "#22d3ee",fontWeight:700,lineHeight:2.4}}>
          ax + by + c = 0 (a²+b²≠0)
        </div>
    ...
    <section id="k2" style={{scrollMarginTop:80,marginBottom:64}}><SH icon="📖" title={t("2. Các Dạng Phương Trình","2. Forms of Line Equations")} />
      <div className="reveal" data-reveal data-reveal-stagger data-stagger="80" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))",gap:16,transition:"all 0.3s"}}>
        {[{name:t("PT tổng quát","General"),formula:"ax + by + c = 0",note:t("Vectơ pháp →n=(a,b)","Normal →n=(a,b)"),c:"#38bdf8",bg:"rgba(14, 165, 233, 0.15)"},
          {name:t("PT tham số","Parametric"),formula:"x=x₀+at, y=y₀+bt (t∈ℝ)",note:t("Vectơ chỉ phương →u=(a,b)","Direction →u=(a,b)"),c:"#4ade80",bg:"rgba(16, 185, 129, 0.15)"},
          {name:t("PT chính tắc","Standard"),formula:"(x−x₀)/a = (y−y₀)/b",note:t("Qua M(x₀,y₀), hướng (a,b)","Through M(x₀,y₀), dir (a,b)"),c:"#fbbf24",bg:"rgba(245, 158, 11, 0.15)"},
          {name:t("PT góc tung độ (y=mx+n)","Slope-intercept"),formula:"y = mx + n",note:t("m=−a/b là hệ số góc","m=−a/b is slope"),c:"#f87171",bg:"rgba(239, 68, 68, 0.15)"},
        ].map((card,i)=>(<article key={i} style={{padding:16,borderRadius:10,background: "rgba(255, 255, 255, 0.04)", border: "1px solid rgba(255, 255, 255, 0.08)",boxShadow:"0 4px 12px rgba(0,0,0,0.1)"}}><div style={{fontSize:13,fontWeight:700,color:card.c,marginBottom:6}}>{card.name}</div><div style={{fontFamily:"monospace",fontSize:14,background:card.bg,color:card.c,padding:"6px 10px",borderRadius:6,marginBottom:4}}>{card.formula}</div><div style={{fontSize:12,color: "rgba(255, 255, 255, 0.5)"}}>{card.note}</div></article>))}
      </div>
    </section>
