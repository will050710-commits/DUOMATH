    ...
    <section id="congThuc" style={{scrollMarginTop:80,marginBottom:64}}><SH icon="📐" title={t("Bảng Công Thức & Nghiệm","Formula & Solution Table")} />
      <div className="reveal" data-reveal style={{overflowX:"auto",borderRadius:10,boxShadow:"0 4px 12px rgba(0,0,0,0.1)"}}>
        <table style={{borderCollapse:"collapse",width:"100%",fontSize:13,minWidth:600}}>
          <thead><tr style={{background:"#22d3ee",color:"white"}}>{["Điều kiện","f>0","f≥0","f<0","f≤0"].map((h,i)=><td key={i} style={{padding:"10px 12px",textAlign:"center",fontWeight:700,border:"1px solid rgba(255,255,255,0.2)"}}>{h}</td>)}</tr></thead>
          <tbody>
            {[["a>0, Δ>0","(−∞,x₁)∪(x₂,+∞)","(−∞,x₁]∪[x₂,+∞)","(x₁,x₂)","[x₁,x₂]"],
              ...
              ["a<0, Δ=0","∅","{x₀}","ℝ\{x₀}","ℝ"],
              ["a<0, Δ<0","∅","∅","ℝ","ℝ"]
            ].map((row,ri)=>(<tr key={ri} style={{background:ri%2===0?"#f9f9f9":"white"}}>{row.map((cell,ci)=>(<td key={ci} style={{padding:"8px 12px",textAlign:"center",border:"1px solid #e0e0e0",fontFamily:"monospace",fontSize:12,fontWeight:ci===0?700:400,color:ci===0?"#22d3ee":"#333"}}>{cell}</td>))}</tr>))}
          </tbody>
        </table>
      </div>
    </section>
