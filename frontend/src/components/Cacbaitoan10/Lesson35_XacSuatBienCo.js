...
export default function Lesson35_XacSuatBienCo() {
  ...
  const tfC=[{'s': 'P(A)=n(A)/n(Ω) chỉ đúng khi các kết quả đồng khả năng.', 'a': true, 'ex': 'ĐÚNG — công thức cổ điển áp dụng cho không gian mẫu đồng khả năng.'}, {'s': '0 ≤ P(A) ≤ 1 với mọi biến cố A.', 'a': true, 'ex': 'ĐÚNG — xác suất luôn trong [0,1].'}, {'s': 'P(A) + P(Ā) = 2.', 'a': false, 'ex': 'SAI — P(A)+P(Ā)=1.'}, {'s': 'P(A∩B)=P(A)·P(B) đúng khi A và B độc lập.', 'a': true, 'ex': 'ĐÚNG — nhân xác suất khi độc lập.'}, {'s': 'P(∅) = 1.', 'a': false, 'ex': 'SAI — P(∅)=0 (biến cố không thể xảy ra).'}];
  const fQ=[{'id': 'f1', 'tp': 'P(A) = n(A) / ___', 'ans': 'n(Ω)', 'alt': ['n(omega)', 'n(O)', 'n(Ω)'], 'h': ''}, {'id': 'f2', 'tp': 'P(Ā) = 1 − ___', 'ans': 'P(A)', 'alt': ['P(A)', 'p(a)'], 'h': ''}, {'id': 'f3', 'tp': "Tung xúc xắc: P('số chẵn') = ___", 'ans': '1/2', 'alt': ['1/2', '0.5', '3/6'], 'h': 'A={2,4,6}'}];
  ...
  useEffect(() => {
    if (md && user) {
      saveGameResult({
        lesson_slug: "Lesson35_XacSuatBienCo",
        mode: "mc",
        score: msc,
        total: mcQ.length
      });
  ...
  return(<div style={{width:"100%",background:"#fff",display:"flex",justifyContent:"center"}}><div style={{width:"1200px",maxWidth:"95%",color:"black",paddingTop:60,paddingBottom:80}}>
    ...
    <header className="reveal" data-reveal style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"20px 0",position:"relative",zIndex:300}}>
      <div><div style={{fontWeight:"bold",fontSize:22,color:"#0B4F5C"}}>Chương X · Xác Suất</div><div style={{fontSize:28,fontWeight:600,marginTop:4}}>Bài 37: Xác Suất Biến Cố</div></div>
    ...
    <div className="reveal" data-reveal data-reveal-stagger data-stagger="60" style={{marginBottom:40,padding:20,borderRadius:10,background:"#f9f9f9",boxShadow:"0 4px 12px rgba(0,0,0,0.1)"}}>
      <div style={{fontSize:18,fontWeight:600,marginBottom:14}}>🎯 {t("Yêu cầu cần đạt","Objectives")}</div>
              <div key={"0"} style={{fontSize:15,color:"#555",marginBottom:6}}>• Định nghĩa và tính P(A)=n(A)/n(Ω). / Define and compute P(A)=n(A)/n(Ω).</div>
        <div key={"1"} style={{fontSize:15,color:"#555",marginBottom:6}}>• Nắm tính chất: 0≤P(A)≤1, P(Ω)=1, P(∅)=0. / Know properties: 0≤P(A)≤1, P(Ω)=1, P(∅)=0.</div>
        <div key={"2"} style={{fontSize:15,color:"#555",marginBottom:6}}>• Áp dụng quy tắc cộng và nhân xác suất. / Apply addition and multiplication rules.</div>
        <div key={"3"} style={{fontSize:15,color:"#555",marginBottom:6}}>• Tính P(Ā)=1−P(A). / Compute P(Ā)=1−P(A).</div>
        <div key={"4"} style={{fontSize:15,color:"#555",marginBottom:6}}>• Giải bài toán xác suất thực tế. / Solve real probability problems.</div>
    </div>
    ...
    <div className="reveal" data-reveal style={{textAlign:"center",color:"#777",fontSize:15,marginBottom:60}}>Toán 10 · Chân Trời Sáng Tạo · Bài 37 / Chương X</div>
