...
export default function Lesson27_NhiThucNewton() {
  ...
  const tfC=[{'s': '(a+b)ⁿ có n+1 số hạng.', 'a': true, 'ex': 'ĐÚNG — từ T₁ (k=0) đến T_{n+1} (k=n).'}, {'s': 'Hệ số của T_{k+1} trong (a+b)ⁿ là Cₙᵏ.', 'a': true, 'ex': 'ĐÚNG — T_{k+1}=Cₙᵏ·aⁿ⁻ᵏ·bᵏ.'}, {'s': 'Σ Cₙᵏ = n!.', 'a': false, 'ex': 'SAI — Σ Cₙᵏ = 2ⁿ (không phải n!).'}, {'s': '(1+x)³ = 1+3x+3x²+x³.', 'a': true, 'ex': 'ĐÚNG — C₃⁰+C₃¹x+C₃²x²+C₃³x³=1+3x+3x²+x³.'}, {'s': 'Trong Tam Giác Pascal, mỗi số bằng tích của hai số phía trên.', 'a': false, 'ex': 'SAI — bằng TỔNG (không phải tích) hai số phía trên.'}];
  const fQ=[{'id': 'f1', 'tp': '(a+b)ⁿ: số hạng tổng quát T_{k+1} = Cₙᵏ · aⁿ⁻ᵏ · ___', 'ans': 'bᵏ', 'alt': ['b^k', 'bk', 'b ᵏ'], 'h': ''}, {'id': 'f2', 'tp': 'Σₖ₌₀⁵ C₅ᵏ = ___', 'ans': '32', 'alt': ['32'], 'h': '2⁵=32'}, {'id': 'f3', 'tp': 'Hệ số x² trong (x+1)³ là ___', 'ans': '3', 'alt': ['3'], 'h': 'C₃²=3'}];
  ...
  useEffect(() => {
    if (md && user) {
      saveGameResult({
        lesson_slug: "Lesson27_NhiThucNewton",
        mode: "mc",
        score: msc,
        total: mcQ.length
      });
  ...
  return(<div style={{width:"100%",background:"#fff",display:"flex",justifyContent:"center"}}><div style={{width:"1200px",maxWidth:"95%",color:"black",paddingTop:60,paddingBottom:80}}>
    ...
    <header className="reveal" data-reveal style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"20px 0",position:"relative",zIndex:300}}>
      <div><div style={{fontWeight:"bold",fontSize:22,color:"#0B4F5C"}}>Chương VIII · Tổ Hợp</div><div style={{fontSize:28,fontWeight:600,marginTop:4}}>Bài 27: Nhị Thức Newton</div></div>
  ...
  <div className="reveal" data-reveal data-reveal-stagger data-stagger="80" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:16,transition:"all 0.3s"}}>
    ...
    <div className="reveal" data-reveal style={{textAlign:"center",color:"#777",fontSize:15,marginBottom:60}}>Toán 10 · Chân Trời Sáng Tạo · Bài 27 / Chương VIII</div>
