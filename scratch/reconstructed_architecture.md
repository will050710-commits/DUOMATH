 ...
 ### D. Gia Sư AI Thông Minh Tích Hợp LightRAG (DuoMCB v2)
 
 #### 1. Mô tả tính năng
 DuoMCB (DuoMath Conversational Math Buddy) là chatbot AI chuyên biệt dành riêng cho giáo dục toán học THPT, tích hợp cơ chế LightRAG (Retrieval-Augmented Generation dựa trên Đồ thị Tri thức). Khi người dùng gửi câu hỏi (dạng văn bản hoặc ảnh chụp), hệ thống sẽ trích xuất thực thể và thực hiện truy vấn song song trên Knowledge Graph cục bộ (`MATH_CONCEPT_GRAPH`) để tìm các khái niệm liên quan, định nghĩa chuẩn, công thức LaTeX và ví dụ minh họa. Việc này giúp AI có dữ liệu nền tảng tin cậy trước khi sinh lời giải, giúp nâng độ chính xác lên 94% và giảm thiểu tối đa hiện tượng ảo tưởng (hallucination) của LLM.
 
 #### 2. Hình ảnh giao diện
 - **Giao diện chat**: Khung chat DuoMCB được thiết kế theo phong cách glassmorphism tối màu, cho phép nhập văn bản, gửi tệp hình ảnh bài tập trực quan. Kết quả trả về hiển thị công thức LaTeX mượt mà thông qua thư viện KaTeX kèm theo các câu hỏi dẫn dắt theo phương pháp Socratic.
 - **Tập tin hình ảnh đính kèm**: `duomcb_chat_page_1782026228425.png` hiển thị toàn bộ cửa sổ chat và lời giải mẫu của AI.
 
 #### 3. Hình ảnh code (Đoạn mã nguồn xử lý RAG)
 Dưới đây là mã nguồn của hàm truy xuất thông tin từ Knowledge Graph trên Backend:
 ...
 def retrieve_math_context(query: str) -> str:
     ...
     if matched_ids:
         return "=== KNOWLEDGE GRAPH (Local Mode) ===\n" + "\n".join(local_contexts) + "\n=== GLOBAL CONTEXT ===\n" + global_ctx + "="*50 + "\n"
     return "=== GLOBAL CONTEXT ===\n" + global_ctx + "="*50 + "\n"
 ```
 
 #### 4. Giải thích code
 - **extract_graph_entities(query)**: Phân tích cú pháp của câu hỏi nhập vào từ người dùng để phát hiện các thực thể có trong đồ thị tri thức (ví dụ: "parabol", "cosin", "cauchy").
 - **Local Context Retrieval**: Duyệt qua danh sách thực thể khớp để trích xuất trực tiếp `definition`, `formulas`, và `examples` của nút đó trong graph dữ liệu `MATH_CONCEPT_GRAPH`.
 - **Edge Relation Traversal**: Duyệt qua các mối quan hệ cạnh (`edges`) để tìm các nút lân cận có quan hệ mật thiết và tự động đề xuất thêm "Khái niệm lân cận liên quan", tạo liên kết logic đầy đủ cho câu trả lời.
 - **Global Context Integration**: Chèn thêm chỉ dẫn ép buộc mô hình LLM luôn sử dụng KaTeX để format biểu thức toán học và áp dụng phương pháp Socratic đặt câu hỏi dẫn dắt người học thay vì cung cấp ngay đáp số cuối cùng.
 ...
 **Styles (File [DuoTranslate.module.css](file:///c:/Users/Latitude%207300/OneDrive/M%C3%A1y%20t%C3%ADnh/duosteam%20-%20Copy/duosteam/frontend/src/components/DuoMCB/DuoTranslate.module.css)):**
 ...
 - **Mô tả tính năng**: Học sinh có thể click trực tiếp vào biểu tượng đóng băng `🧊` trên thanh công cụ `StreakBar` hiển thị ở Header để mua lượt đóng băng bằng XP của mình nhằm bảo vệ chuỗi ngày học liên t