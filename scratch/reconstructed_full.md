...
### B. Bảo Vệ Dữ Liệu
  ...
  - **Luyện tập (Practice Ring - màu Cyan)**: Đích nhắm 10 bài tập/ngày.
  - **Tinh thông (Mastery Ring - màu Tím)**: Đích nhắm làm đúng 3 câu khó mức Thông hiểu (TH), Vận dụng (VD), hoặc Vận dụng cao (VDC).
  - **Học thuật (Socratic Ring - màu Hồng)**: Đích nhắm 5 lần tra cứu từ vựng toán học qua DuoTranslate hoặc trò chuyện học tập với trợ lý DuoMCB.
- **Hình ảnh giao diện**: Component `MasteryRings` hiển thị các vòng chỉ tiêu SVG đồng tâm bo gọn, phát sáng, đặt ngay trung tâm bảng điều khiển tiến trình của người dùng trên trang chủ.
- **Đoạn mã code tiêu biểu**:
  ```javascript
  ...
  // Vẽ các vòng tròn đồng tâm SVG sử dụng framer-motion:
  <motion.circle
    cx={center} cy={center} r={ring.r} fill="transparent"
  ...
  />
  ```
- **Giải thích code**: Hàm tự động tính tỷ lệ phần trăm hoàn thành, chuyển đổi sang giá trị `strokeDashoffset` tương ứng của thuộc tính SVG stroke để hiển thị hiệu ứng vẽ vòng tròn mượt mà trên UI.

#### 2. Đổi XP lấy Đóng Băng Streak (Streak Freeze) từ StreakBar
- **Mô tả tính năng**: Học sinh có thể click trực tiếp vào biểu tượng đóng băng `🧊` trên thanh công cụ `StreakBar` hiển thị ở Header để mua lượt đóng băng bằng XP của mình nhằm bảo vệ chuỗi ngày học liên tục khi bận rộn.
- **Hình ảnh giao diện**: Biểu tượng `🧊` luôn hiển thị ở góc phải StreakBar kèm số lượt hiện tại. Khi click, một pop-up xác nhận trực quan hiển thị và trừ 100 XP để tăng lượt Đóng băng.
- **Đoạn mã code tiêu biểu**:
  ```python
  ...
  async def buy_streak_freeze(request: Request):
      ...
      try:
          ...
          if total_xp < cost:
              raise HTTPException(400, "Không đủ XP!")
          db.execute(
          ...
          db.execute("UPDATE user_gamification SET freeze_count = freeze_count + 1 WHERE user_id=?", (uid,))
          db.commit()
          ...
  ```
- **Giải thích code**: Endpoint API `/api/gami/buy-freeze` nhận request từ client, kiểm tra XP hiện tại của người dùng. Nếu đủ, chèn bản ghi âm điểm để khấu trừ XP một cách an toàn và cộng thêm 1 lượt đóng băng streak vào bảng `user_gamification`.

#### 3. Hệ thống Gacha mở rương công thức & Album thẻ bài
- **Mô tả tính năng**: Cho phép học sinh đổi 50 XP để mở "Rương Tri thức" nhận ngẫu nhiên một thẻ bài công thức Toán THPT. Mỗi thẻ bài bao gồm tiêu đề, công thức LaTeX chuẩn, và lời giải nghĩa thực tế của nó. Toàn bộ thẻ bài được tổng hợp tại một Album kỹ thuật số phân loại theo 3 mức độ hiếm (Common, Rare, Legendary) đi kèm hiệu ứng hover 3D.
- **Hình ảnh giao diện**: Hộp quà mở rương `🎁` động trên trang chủ kèm hiệu ứng rung chuyển. Khi lật mở, thẻ bài lấp lánh hiện ra đầy lôi cuốn.
- **Đoạn mã code tiêu biểu**:
  ```python
  ...
  async def open_gacha_chest(request: Request):
      ...
      card_id = card["id"]
      # Insert or update user_cards
      ...
  ```
- **Giải thích code**: Hàm backend chọn ngẫu nhiên một thẻ trong danh sách công thức, ghi vào bảng `user_cards` để lưu trữ bộ sưu tập của học sinh. Ở frontend, component `KnowledgeAlbum` gọi endpoint `/api/gacha/collection` để lấy danh sách thẻ và dùng KaTeX hiển thị.

#### 4. Bảng xếp hạng cục bộ theo Trường và Lớp học (Local/School Leaderboard)
- **Mô tả tính năng**: Bộ lọc bảng xếp hạng cải tiến cho phép học sinh cạnh tranh trực tiếp trong nhóm bạn bè cùng lớp hoặc cùng trường THPT, tạo động lực thi đua học tập cục bộ lành mạnh.
- **Hình ảnh giao diện**: Bảng xếp hạng mini tích hợp ngay trên trang chủ với 3 tab chuyển đổi: "Toàn cầu", "Trường học", "Lớp học" hoạt động mượt mà.
- **Đoạn mã code tiêu biểu**:
  ```python
  ...
  async def get_leaderboard(request: Request):
      ...
      # Tự động chèn WHERE u.school = ? hoặc u.grade = ? vào câu lệnh SQL truy vấn tổng điểm...
  ```
- **Giải thích code**: API `/api/leaderboard` lọc trực tiếp học sinh theo thuộc tính `school` hoặc `grade` truyền từ Client lên qua các URL query parameters, trả về Top 50 người dẫn đầu tương ứng để hiển thị trên UI.
...
## 13. Tính Năng Mới & Nâng Cấp Hệ Thống (Tháng 6, 2026)
 
### A. AI MathMap Questions Parser (Tự động chuyển hóa câu hỏi bằng AI)
 
#### 1. Mô tả tính năng
Học sinh/Giáo viên có thể thiết kế một bộ câu hỏi kiểm tra nhanh chóng thông qua việc tải lên tệp tin văn bản thuần túy `.txt` hoặc `.md`. AI sẽ phân tích cấu trúc câu hỏi, các lựa chọn, đáp án đúng và lời giải chi tiết để tự động điền các trường dữ liệu trong công cụ **MathMapCreator**. Điều này mang lại sự tiện lợi, nhanh chóng, loại bỏ việc gõ thủ công từng câu hỏi.
 
#### 2. Quy tắc định dạng tệp tin đầu vào
Để hệ thống nhận diện chính xác, tệp tin cần tuân thủ cấu trúc sau:
- **Câu hỏi**: Bắt đầu bằng từ khóa `Câu [Số]:` hoặc `Question [No]:`
- **Các phương án lựa chọn**: Sử dụng các ký tự `A.`, `B.`, `C.`, `D.` ở đầu dòng mới.
- **Đáp án đúng**: Xác định qua dòng `Đáp án: [A/B/C/D]` hoặc `Answer: [A/B/C/D]`.
- **Lời giải (Tùy chọn)**: Bắt đầu bằng `Giải thích:` hoặc `Explanation:`. Các công thức toán học được bọc bằng ký hiệu LaTeX `$` (ví dụ: `$y = x^2 - 4x + 3$`).
 
#### 3. Cấu trúc Code chính & Giải thích
 
**Backend (File [main.py](file:///c:/Users/Latitude%207300/OneDrive/M%C3%A1y%20t%C3%ADnh/duosteam%20-%20Copy/duosteam/backend/main.py)):**
Thêm endpoint `POST /api/mathmap/parse-file` nhận vào đoạn text thô từ file tải lên và chuyển hóa dữ liệu qua hai tầng:
1. *Tầng cục bộ (Local Regex Parser)*: Phân tích cú pháp bằng biểu thức chính quy (Regular Expressions) để nhận diện nhanh các câu hỏi tuân thủ đúng template. Đây là cơ chế chạy offline trực tiếp, cực kỳ nhanh chóng và chính xác 100% khi cấu trúc tệp tin đúng mẫu chuẩn.
2. *Tầng AI Fallback (Llama-3.1 via Groq)*: Nếu tầng cục bộ không phát hiện câu hỏi nào (hoặc tệp tin có bố cục tự do), server sẽ gửi toàn bộ văn bản lên Groq API gọi mô hình `llama-3.1-8b-instant` thực hiện trích xuất ngữ nghĩa và trả về cấu trúc JSON chuẩn hóa.
 
```python
def parse_math_questions_local(text: str) -> dict:
    # Trích xuất trực tiếp câu hỏi bằng Regex cục bộ
    # Đọc tiêu đề, mô tả, và lặp qua các câu hỏi dạng:
    # Câu 1: [Nội dung] A. [Lựa chọn] B. [Lựa chọn] Đáp án: A
    ...
```
 
**Frontend (File [MathMapCreator.js](file:///c:/Users/Latitude%207300/OneDrive/M%C3%A1y%20t%C3%ADnh/duosteam%20-%20Copy/duosteam/frontend/src/components/mrm/MathMapCreator.js)):**
Tích hợp nút bấm **🧠 Nhập bằng AI (.txt, .md)** đi kèm với input file ẩn. Khi người dùng chọn tệp tin:
1. `FileReader` đọc nội dung tệp tin dưới dạng text thuần túy.
2. Thiết lập trạng thái `aiParsing = true` để hiển thị hiệu ứng xoay tròn tải dữ liệu (`loading spinner`).
3. Gửi payload lên API `/api/mathmap/parse-file`.
4. Nhận về JSON và tự động cập nhật các React states `metadata` và `questions`, điền đầy đủ thông tin vào giao diện chỉnh sửa bản đồ học tập.
...
### B. Bilingual DuoTranslate & MathGraphSVG Integration (Dịch thuật song ngữ & Đồ thị minh họa)
 
#### 1. Mô tả tính năng
DuoTranslate được nâng cấp toàn diện giúp người dùng bôi đen bất kỳ thuật ngữ hoặc văn bản toán học nào để:
- Tự động nhận diện ngôn ngữ nguồn (Tiếng Việt hoặc Tiếng Anh).
- Thực hiện dịch thuật hai chiều (`EN ➔ VI` và `VI ➔ EN`).
- Cung cấp nút bấm **Xem thêm lý thuyết & đồ thị** để mở rộng ngăn kéo thông tin.
- Hiển thị bài giảng lý thuyết chi tiết bằng cả hai ngôn ngữ (cho phép chuyển đổi ngôn ngữ VN/EN bằng tab chọn) với các công thức toán học phức tạp được render sắc nét qua thư viện **KaTeX**.
- Hiển thị hình vẽ/đồ thị hình học phẳng và hàm số động từ component **MathGraphSVG** tương ứng với loại thuật ngữ được chọn (`parabola`, `venn`, `trig`, `vectors`, `ellipse`, `inequality`).
 
#### 2. Cấu trúc Code chính & Giải thích
 
**Backend (File [main.py](file:///c:/Users/Latitude%207300/OneDrive/M%C3%A1y%20t%C3%ADnh/duosteam%20-%20Copy/duosteam/backend/main.py)):**
API `/api/translate` được thiết kế lại để nhận diện ngôn ngữ đầu vào và phân loại thuật ngữ toán học (`diagram_type`). Để ứng dụng chạy mượt mà ở môi trường offline hoặc khi thiếu Groq API Key, server tích hợp một từ điển cục bộ mẫu (Mock database) chứa đầy đủ lý thuyết và cấu hình đồ thị của 6 dạng toán hình học phẳng chính:
 
```python
def get_mock_translation(text: str) -> dict:
    t = text.lower()
    if any(k in t for k in ["parabol", "quadratic", "hàm số bậc hai"]):
        return {
            ...
            "theory": {
                ...
                "en": "The quadratic function $y = ax^2 + bx + c$ has its vertex at $I\\left(-\\frac{b}{2a}, -\\frac{\\Delta}{4a}\\right)..."
            },
            ...
        }
    ...
...
**Frontend (File [DuoTranslate.js](file:///c:/Users/Latitude%207300/OneDrive/M%C3%A1y%20t%C3%ADnh/duosteam%20-%20Copy/duosteam/frontend/src/components/DuoMCB/DuoTranslate.js)):**
- Tích hợp hàm `parseMathAndText` để bóc tách các đoạn chữ thường và các đoạn công thức LaTeX nằm giữa các ký hiệu `$` hoặc `$$`.
- Sử dụng hàm `renderMathText` gọi thư viện `katex.renderToString` để hiển thị trực tiếp công thức toán.
- Thêm các trạng thái React `showTheory` (ẩn/hiện chi tiết lý thuyết) và `theoryLang` (ngôn ngữ lý thuyết hiện tại: `vi` hoặc `en`).
- Render component `<MathGraphSVG type={results.diagram_type} />` bên trong thẻ minh họa để vẽ hình học phẳng trực quan với các chuyển động mượt mà bằng thư viện Framer Motion.
 
**Styles (File [DuoTranslate.module.css](file:///c:/Users/Latitude%207300/OneDrive/M%C3%A1y%20t%C3%ADnh/duosteam%20-%20Copy/duosteam/frontend/src/components/DuoMCB/DuoTranslate.module.css)):**
Bổ sung các CSS rules cho bảng lý thuyết chi tiết, bộ chọn ngôn ngữ (`.theoryLangSelector`), và khung chứa đồ thị (`.diagramBox`) tạo cảm giác tối màu sâu thẳm, viền bo tròn phát sáng nhẹ đúng phong cách thiết kế Premium của DuoMath.
 
 
### C. Các Tính Năng Humane Gamification Nâng Cao (Bổ Sung Tháng 6, 2026)

Hệ thống DuoMath được bổ sung 4 tính năng gamification cao cấp hướng tới việc thúc đẩy thói quen học tập bền bỉ của học sinh mà không gây áp lực tiêu cực hay làm xáo trộn dữ liệu điểm số, ELO hiện tại.

#### 1. Daily Mastery Rings (Vòng chỉ tiêu ngày đồng tâm)
- **Mô tả tính năng**: Hệ thống theo dõi 3 chỉ tiêu học tập hàng ngày dưới dạng 3 vòng tròn đồng tâm SVG phát sáng chuyển động động. Mỗi vòng đại diện cho một loại hoạt động:
  - **Luyện tập (Practice Ring - màu Cyan)**: Đích nhắm 10 bài tập/ngày.
  - **Tinh thông (Mastery Ring - màu Tím)**: Đích nhắm làm đúng 3 câu khó mức Thông hiểu (TH), Vận dụng (VD), hoặc Vận dụng cao (VDC).
  - **Học thuật (Socratic Ring - màu Hồng)**: Đích nhắm 5 lần tra cứu từ vựng toán học qua DuoTranslate hoặc trò chuyện học tập với trợ lý DuoMCB.
- **Hình ảnh giao diện**: Component `MasteryRings` hiển thị các vòng chỉ tiêu SVG đồng tâm bo gọn, phát sáng, đặt ngay trung tâm bảng điều khiển tiến trình của người dùng trên trang chủ.
